...
$(function(){
	$('#dg').datagrid({    
	    url:'<%=request.getContextPath()%>/shopaction/selectshopxiaolei',
...
function addshop(){
	...
	$("#dialog_addshop").dialog({
		 ...
		 title: '新增商品小类',    
		    ...
		    closed: false,    
		    cache: false,
		    href:"<%=request.getContextPath()%>/shopaction/toaddshopxiaolei",
		    modal: true ,
		    toolbar:[{
				text:'保存',
				iconCls:'icon-edit',
				handler:function(){
					...
					$.ajax({
						url:"<%=request.getContextPath()%>/shopaction/addshopxiaolei",
...
function deleteshop(){
	...
	if(ids.length!=1){
		  $.messager.alert('警告','请选择1项'); 
				...
				$.ajax({
				url:"<%=request.getContextPath()%>/shopaction/deleteshopxiaoleibyid?id="+ids[0].id,
...
function updateshop(){
	  ...
	  if(ids.length!=1){
		  ...
		  $("#dialog_update").dialog({
		      ...
		      closed: false,    
		      cache: false,
		      href:"<%=request.getContextPath()%>/shopaction/selectshopxiaoleibyid?id="+ids[0].id,
		    toolbar:[{
				text:'保存',
				iconCls:'icon-edit',
				handler:function(){
