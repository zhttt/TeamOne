...
<script type="text/javascript">
	...
	$('#dg1').datagrid({    
	    url:'<%=request.getContextPath()%>/shopaction/queryshopdalei',
...
function addshop1(){
	...
	$("#dialog_addshop1").dialog({
		 ...
		 title: '新增商品大类',    
		    ...
		    closed: false,    
		    cache: false,
		    href:"<%=request.getContextPath()%>/shopaction/toaddshopdalei",
		    modal: true ,
		    toolbar:[{
				text:'保存',
				iconCls:'icon-edit',
				handler:function(){
					...
					$.ajax({
						url:"<%=request.getContextPath()%>/shopaction/addshopdalei",
...
function deleteshop1(){
	...
	if(ids.length!=1){
		  $.messager.alert('警告','请选择1项'); 
				...
				$.ajax({
				url:"<%=request.getContextPath()%>/shopaction/deleteshopdaleibyid?id="+ids[0].id,
    ...
    function updateshop1(){
        ...
        if(ids.length!=1){
            ...
            $("#dialog_update1").dialog({
                ...
                closed: false,
                cache: false,
                href:"<%=request.getContextPath()%>/shopaction/selectshopdaleibyid?id="+ids[0].id,
                toolbar:[{
                    text:'保存',
                    iconCls:'icon-edit',
                    handler:function(){
